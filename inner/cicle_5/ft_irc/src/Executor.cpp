...
static bool isValidNickname(const std::string &nickname) {
	...
	for (size_t i = 1; i < nickname.size(); i++)
		if (!std::isalnum(nickname[0]) && ! isSpecialChar(nickname[0]))  return false;
	return true;
}
...
void Executor::_execute_topic(Server &server, Connection *&node, const Message &msg)
{
	...
	std::string	setat = utils::time_to_str(chan->topic_time);
	...
	if (msg.params.size() == 1)
	{
		...
		else
		{
			...
			dispatch_packet(server, node, BUILD_RPL_TOPICWHOTIME(server.servername, node->nickname, channame, chan->topic_setter, setat));
		}
		return;
	}
...
void Executor::_execute_mode(Server &server, Connection *&node, const Message &msg)
{
    ...
    if (is_channel)
    {
            ...
            switch (mode)
            {
                ...
                case 'k':
                    if (adding)
                    {
                        ...
                        channel->key = msg.params[param_index++];
                ...
                case 'l':
                    if (adding)
                    {
                        ...
                        channel->limit = std::stoi(msg.params[param_index++]);
		...
		dispatch_packet(server, node, BUILD_RPL_CHANNELMODEIS(server.servername, node->nickname, channel->name, channel->get_mode_string()));
    }
    else
    {
